...
public class CMainForm extends javax.swing.JFrame implements Runnable{
    ...
    public CMainForm(String title) throws HeadlessException {
        ...
        restartButton.addActionListener(acionEvent -> buttonRestartClick());
    }
    ...
    private void buttonStartClick(){
        ...
        else setTitle("Kółko - krzyżyk: [X]");
    }

    private void buttonRestartClick() { CConfig.reset();
    ...
    private void mousePressedEvent(MouseEvent e){if (CConfig.accepted) {
        if (CConfig.yourTurn) {
    ...
    private void initializeServer(){
        try{
            serverSocket = new ServerSocket(CConfig.port, 8,
                    InetAddress.getByName(CConfig.ip));
        }catch (Exception e){
            e.printStackTrace();
        }
        CConfig.yourTurn = true;
    ...
    public boolean checkWin(boolean myWin){
        String str;
        if(myWin){
            if(CConfig.circle)  str="O"; else str = "X";
        }else {
            if (CConfig.circle) str="X"; else str = "O";
        }
